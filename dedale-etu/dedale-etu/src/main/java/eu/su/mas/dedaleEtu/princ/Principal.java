...
/**
 * This class is used to start the platform and the agents. To launch your
 * agents in the environment you desire you will have to :
 * <ul>
 * <li>set the ConfigurationFile parameters (and maybe update the files in
 * resources/) {@link ConfigurationFile}</li>
 * <li>create your agents classes and call them in the createAgents method below
 * </li>
 * </ul>
 * 
 * @author hc
...
public class Principal {
	...
	public static void main(String[] args) {
		...
		if (ConfigurationFile.COMPUTERisMAIN) {
			// Whe should create the Platform and the GateKeeper, whether the platform is
			// distributed or not

			// 1), create the platform (Main container (DF+AMS) + containers + monitoring
			// agents : RMA and SNIFFER)
			rt = emptyPlatform(containerList);

			// 2) create the gatekeeper (in charge of the environment), the agents, and add
			...
			// 1') If a distant platform already exist and you want to create and connect
			// your container to it
			containerList.putAll(createAndConnectContainer(ConfigurationFile.LOCAL_CONTAINER_NAME,
	...
	private static Runtime emptyPlatform(HashMap<String, ContainerController> containerList) {
		...
		Profile pMain = new ProfileImpl(ConfigurationFile.PLATFORM_HOSTNAME, ConfigurationFile.PLATFORM_PORT,
		...
		// 3) create monitoring agents : rma agent, used to debug and monitor the
		// platform; sniffer agent, to monitor communications;
	 ...
	 *         <br/>
	 *         note: there is a smarter way to find a container with its name, but
	 *         we go fast to the goal here. Cf jade's doc.
	 */
	private static HashMap<String, ContainerController> createContainers(Runtime rt) {
		...
		HashMap<String, ContainerController> containerList = new HashMap<String, ContainerController>();// bad to do it
																										// here.
		...
		pContainer = new ProfileImpl(ConfigurationFile.PLATFORM_HOSTNAME, ConfigurationFile.PLATFORM_PORT,
				ConfigurationFile.PLATFORM_ID);
		pContainer.setParameter(Profile.CONTAINER_NAME, containerName);
		System.out.println("Launching container " + pContainer);
		containerRef = rt.createAgentContainer(pContainer); // ContainerController replace AgentContainer in the new
															// versions of Jade.
	 ...
	 * 
	 * @param containerName
	 * @param host          is the IP of the host where the main-container should be
	 *                      listen to. A null value means use the default (i.e.
	 *                      localhost)
	 * @param platformID    is the symbolic name of the platform, if different from
	 *                      default. A null value means use the default (i.e.
	 *                      localhost)
	 * @param port          (if null, 8888 by default)
	 * @return
	 */
	private static HashMap<String, ContainerController> createAndConnectContainer(String containerName, String host,
		...
		System.out.println("Create and Connect container " + containerName + " to the host : " + host + ", platformID: "
	...
	private static List<AgentController> createAgents(HashMap<String, ContainerController> containerList) {
		...
		if (ConfigurationFile.COMPUTERisMAIN) {
			...
			try {
				...
				// Object[] objtab=new
				// Object[]{ConfigurationFile.INSTANCE_TOPOLOGY,ConfigurationFile.INSTANCE_CONFIGURATION_ELEMENTS,ConfigurationFile.ENVIRONMENT_TYPE};
				Object[] objtab = new Object[] { ConfigurationFile.INSTANCE_TOPOLOGY,
						ConfigurationFile.INSTANCE_CONFIGURATION_ELEMENTS, ConfigurationFile.ENVIRONMENT_TYPE,
						ConfigurationFile.ENVIRONMENTisGRID, ConfigurationFile.ENVIRONMENT_SIZE,
						ConfigurationFile.ACTIVE_DIAMOND, ConfigurationFile.ACTIVE_GOLD,
						ConfigurationFile.ACTIVE_WELL };// used to give informations to the agent
		...
		/************************************************************************************************
		 * The main container (now) exist, we deploy the agent(s) on their local
		 * containers They will have to find the gatekeeper's container to deploy
		 * themselves in the environment. This is automatically performed by all agent
		 * that extends AbstractDedaleAgent
		 ************************************************************************************************/
		AgentController ag;

		/*****************************************************
		 * 
		 * ADD YOUR AGENTS HERE - As many as you want. Any agent added here should have
...
//		// entityParameters1);
//		ag = createNewDedaleAgent(c, agentName, CollectorAgent.class.getName(), entityParameters1);
//		agentList.add(ag);
...
//
//		// 4) Give the class name of your agent to let the system instantiate it
//		ag = createNewDedaleAgent(c, agentName, CollectorAgent.class.getName(), entityParameters2);
//		agentList.add(ag);
...
//		
//		//4) Give the class name of your agent to let the system instantiate it
//		ag=createNewDedaleAgent(c, agentName, CollectorAgent.class.getName(), entityParameters3);
//		agentList.add(ag);
...
//		agentList.add(ag);
		 ...
		 *********/
		  ...
		  //1) Get the container where the agent will appear c =
		  containerList.get(ConfigurationFile.LOCAL_CONTAINER2_NAME);
		  Assert.assertNotNull("This container does not exist",c);
...
//		  c = containerList.get(ConfigurationFile.LOCAL_CONTAINER2_NAME);
//		  Assert.assertNotNull("This container does not exist",c); agentName="Explo3";
//		  Object [] entityParameters3= {"My parameters"}; ag =
//		  createNewDedaleAgent(c,agentName,ExploreAgent.class.getName(),entityParameters3);
//		  agentList.add(ag);
	...
	private static AgentController createNewDedaleAgent(ContainerController initialContainer, String agentName,
			String className, Object[] additionnalParameters) {
		// Object[] objtab=new Object[]{env,agentName};//used to give informations to
		// the agent
